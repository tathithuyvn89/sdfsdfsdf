<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button slot="reference" type="danger" @click="handlePopDeleteMany()">{{ $t('button.delete') }}</el-button>
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          ...
          <el-button slot="reference" icon="el-icon-delete" type="danger" circle @click="handlePopDeleteOne(row)" />
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <el-form-item :label="$t('popup.tag.tpClass_en')" prop="name_en" class="inputClassname">
          <el-input v-model="ruleForm.name_en" :placeholder="$t('popup.tag.pClass_en')" />
        </el-form-item>

        <el-form-item :label="$t('popup.tag.tpClass_ja')" prop="name_ja" class="inputClassname">
          <el-input v-model="ruleForm.name_ja" :placeholder="$t('popup.tag.pClass_ja')" />
        </el-form-item>
    ...
    <el-dialog
      :title="$t('popup.tag.delete.title')"
      :visible.sync="dialogVisibleDelete"
      center
      width="30%"
    >
      <center>{{ $t('popup.tag.delete.content') }}</center>
      <div slot="footer" class="dialog-footer">
...
<script>
import { fetchList, deleteTag, addTag, editTag } from '@/api/tag';

export default {
  data() {
    return {
      currentNumberRow: 10, // Number of departments per page
      TagList: [], // The array to contain all the departments on a page
      ...
      rules: { // Rule validate
        name_en: [
          { required: true, message: this.$t('popup.tag.rule.rname'), trigger: ['blur', 'change'] },
          { max: 255, message: this.$t('popup.tag.rule.lname'), trigger: ['blur', 'change'] },
        ],
        name_ja: [
          { required: true, message: this.$t('popup.tag.rule.rname'), trigger: ['blur', 'change'] },
          { max: 255, message: this.$t('popup.tag.rule.lname'), trigger: ['blur', 'change'] },
  ...
  watch: {
    listIdDeleteFaildChange() {
      ...
      this.toggleTableRowClassName(this.$refs.tableTag.$el, this.listIndexDeleteFaild);
    },
  },
  created() {
  ...
  methods: {
          ...
          if (this.tempActions === 'Create') {
            ...
            resCreate.then((data) => {
              if (data.message === 'Record successfully') {
                ...
                this.$message({
                  message: this.$t('popup.tag.notify.deleteSuccess'),
                  type: 'success',
                });
                this.listLoading = true;
                this.TagList = data.page_infor.data;
          ...
          if (this.tempActions === 'Edit') {
            ...
            resEdit.then((data) => {
              if (data.message === 'Edit successfully') {
                ...
                this.dialogVisible = false;
                this.$message({
                  message: this.$t('popup.tag.notify.editSuccess'),
                  type: 'success',
                });
                this.listLoading = true;
                this.TagList = data.page_infor.data;
                ...
                this.dialogVisible = false;
                this.$message({
                  message: this.$t('popup.tag.notify.editFailed'),
                  type: 'error',
                });
              }
            });
    ...
    async handleDelete(row) {
      ...
      await deleteTag({
        ...
        .then((res) => {
          ...
          this.listLoading = true;
          this.TagList = res.page_infor.data;
        ...
        .catch((err) => {
          this.dialogVisibleDelete = false;
          if (err.response.status === 500 && err.response.data.error === 'errorPage.errorTagDelete') {
    ...
    async DeleteMany() {
      ...
      if (this.multipleSelection.length === 0){
        this.dialogVisibleDelete = false;
        this.$message({
          message: this.$t('popup.tag.notify.notchoseDelete'),
          type: 'warning',
        });
    ...
    getListIndexDeleteFaild() {
      ...
      for (var i = 0; i < this.TagList.length; i++) {
        for (var j = 0; j < this.listIdDeleteFaild.length; j++) {
          if (this.TagList[i].id === this.listIdDeleteFaild[j].id) {
            listIndex.push(i);
          }
        }
      }
    ...
    sortTableChange() {
      var selectEl = this.$refs.tableTag.$el;
      var els = selectEl.querySelectorAll(
        '.el-table__body-wrapper > table > tbody > tr'
      );
      // Loop to delete class "danger-row" in el
    ...
    Create() { // Event click buttom create
      ...
      this.titleCnE = this.$t('popup.tag.tCreate');
    },
    handleEdit(row) { // Event click buttom edit
      this.dialogVisible = true;
      this.tempActions = 'Edit';
      this.titleCnE = this.$t('popup.tag.tEdit');
